...
<body>
	...
	<section id="enquiries">
		...
		ArrayList<HashMap> enquiries = enquiries = database.getAllEnquiries();
		database.disconnect();
		...
		<div class="py-3 px-3 my-1">
			<p>
				<b>Name:</b>&nbsp;<%=enquiry.get("name")%>, 
				<b>Phone No:</b>&nbsp;<%=enquiry.get("phone_no")%>&nbsp;
				[<%=enquiry.get("status")%>]
				<%
			if (enquiry.get("status").equals("pending")) {
			%>
				<a class="btn btn-success"
					href="EnquiryStatusUpdate?enquiry_id=<%=enquiry.get("enquiry_id")%>">Done</a>
				<%
				}
				%>
			</p>
